function [SVM_Struct, Features, TrainningResult, TestResult, TrainningSet, TestSet] = Classifier_HOG(Iter_CrossVal)

%import data
Animals = {'dolphin','giraffe','rabbit','sheep'};
Features = zeros(900,59,4);
for ii = 1:4
    Path = [pwd '\FeaturePoints\HOG\' strjoin(Animals(ii)) '\' strjoin(Animals(ii)) '_HOG.mat'];
    Struct = load(Path);
